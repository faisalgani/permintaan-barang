...
class C_room_type extends Controller
{
    ...
    public function getData(Request $request){
        ...
        if (strlen($request->input("search.value")) > 0) {
            $query              = M_type_room::select("*")
            ->orWhere('type', 'like', "%".$request->input("search.value")."%")
    ...
    public function pageRoom($id = null){
        $data = [];
        if($id !== null){
            $query = M_type_room::where("id", "=", $id)->get();
            $data = $query;
        }
        $response = app(\App\Http\Controllers\C_pages::class)->returnTemplate("Formulir", $data);
        return view('cpanel.pages.room_type.index')->with($response);
    }

    public function form($id = null){
        $data = [];
        if($id !== null){
            $query = M_type_room::where("id_type", "=", $id)->get();
            $data = $query;
        }
        $response = app(\App\Http\Controllers\C_pages::class)->returnTemplate("Formulir", $data);
        return view('cpanel.pages.room_type.form')->with($response);
    }
    ...
    public function getStore($id = null){
        ...
        if($id == null){
            $query = M_type_room::orderBy("created_at", "desc")->get();
        }else{
            $query = M_type_room::where("id", "=", $id)->orderBy("created_at", "desc")->get();
        }
        $response['data'] = $query;
    ...
    public function create(Request $request){
        ...
        $params['id_type'] = Uuid::uuid4()->toString();
        ...
        try {
            ...
            $query = M_type_room::create($params);
            if(!$query){
                $response['code'] = 401;
            }else{
                $response['code'] = 200;
    ...
    public function update(Request $request){
        ...
        try {
            //  Block of code to try
            $query = M_type_room::where("id", "=", $params['id']);
