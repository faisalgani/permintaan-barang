...
class LokasiBarangController extends Controller
{
    ...
    public function page($id = null){
        $data = [];
        if($id !== null){
            $query = lokasiBarang::where("id", "=", $id)->get();
            $data = $query;
        }
        $response = app(\App\Http\Controllers\C_pages::class)->returnTemplate("Formulir", $data);
        return view('cpanel.pages.lokasi_barang.index')->with($response);
    }

    public function form($id = null){
        $data = [];
        if($id !== null){
            $query = lokasiBarang::where("id_room", "=", $id)->get();
            $data = $query;
        }
        $response = app(\App\Http\Controllers\C_pages::class)->returnTemplate("Formulir", $data);
        return view('cpanel.pages.lokasi_barang.form')->with($response);
    }
    ...
    public function getStore($id = null){
        ...
        if($id == null){
            $query = lokasiBarang::orderBy("created_at", "ASC")->get();
        }else{
            $query = lokasiBarang::where("id", "=", $id)->orderBy("created_at", "ASC")->get();
        }
        $response['data'] = $query;
    ...
    public function create(Request $request){
        ...
        $key = lokasiBarang::latest()->value('id');
        if($key == null){
            $id= 1;
        }else{
            $id= $key+1;
        ...
        try {
            ...
            $query = lokasiBarang::create($params);
            if(!$query){
                $response['code'] = 401;
            }else{
                $response['code'] = 200;
    ...
    public function update(Request $request){
        ...
        try {
            //  Block of code to try
            $query = lokasiBarang::where("id", "=", $params['id']);
