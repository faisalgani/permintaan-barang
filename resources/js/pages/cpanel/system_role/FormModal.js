...
export default class FormModal extends Component {
        ...
        if($("#id").val() == ""){
            ...
            if(typeof(result.code) !== "undefined"){
                if(result.code == 200){
                    toastr.info(result.message);
                    setTimeout(() => {
                        window.location.href = $this.state.url+"/admin/system_role";
                    }, 1000);
                }else{
                    if(typeof(result.role) !== "undefined"){
                        if(result.role.length > 0){
                            ...
                            await result.role.forEach(element => {
                                this.setState({
                                    [element.key]: {
                                        valid: false,
            ...
            let result = await ExternalAPI.post(
                this.state.url+'/api/v1/system_role/update', 
                this.state.header, 
                await encryptParameter(await this.getParams()));
    ...
    async componentDidMount() {
        var $this = this;
        let menu = await ExternalAPI.get(
            this.state.url+'/api/v1/system_menu/store', 
            this.state.header);
        
        if(typeof(menu.code) !== "undefined"){
            if(menu.code == 200){
                await this.setState({
                    dataMenu: menu.data, 
                });
        ...
        let group = await ExternalAPI.get(
            this.state.url+'/api/v1/system_group/store', 
            this.state.header);
        
        if(typeof(group.code) !== "undefined"){
            if(group.code == 200){
                await this.setState({
    ...
    render() {
        const {dataMenu, dataGroup} = this.state;
        return (
            <>
                <div className="modal fade" id="exampleModal" tabIndex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div className="modal-dialog" role="document">
                        ...
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalLabel">Form role</h5>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="row">
                                <div className="col-md-12">
                                    <div className="form-group">
                                        <label>Group</label>
                                        <select onChange={this.changeGroup} className={this.state.group.valid == true ? "form-control":"form-control is-invalid"} id="group" name="group">
                                            <option key={0} value={""}>---</option>
                                            {dataGroup.map((item) => {
                                                if(this.state.group.value == item.id){
                                                    return <option key={item.id} selected value={item.id}>{item.group}</option>
                                                }else{
                            ...
                            <div className="row">
                                <div className="col-md-12">
                                    <div className="form-group">
                                        <label>Menu</label>
                                        <select onChange={this.changeMenu} className={this.state.menu.valid == true ? "form-control":"form-control is-invalid"} id="menu" name="menu">
                                            <option key={0} value={""}>---</option>
                                            {dataMenu.map((item) => {
                                                if(this.state.menu.value == item.id){
                                                    return <option key={item.id} selected value={item.id}>{item.menu}</option>
                                                }else{
...
if (document.getElementById('cpanel-system_role-form_modal')) {
    const app = document.getElementById('cpanel-system_role-form_modal');
    ReactDOM.render(<FormModal {...app.dataset}/>, document.getElementById('cpanel-system_role-form_modal'));
}
