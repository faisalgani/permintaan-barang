...
export default class DataGrid extends Component {
    ...
    async componentDidMount() {
        var $this = this;
        await this.setState({
            datatable: await $('#dataUsers').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX"   : true,
                "scrollY"   : true,
                "ajax" 		: this.state.url+"/api/v1/users/read",
                ...
                "columns": [
                    { 
                      ...
                      "render": function(data, type, row) {
                        ...
                        "render": function(data, type, row) {
                            var address = "";
                            if(row.address !== null){
                                var html = row.address.replace(/(<([^>]+)>)/gi, "");
                                var limit_char = $this.state.limitChar;
                                var ArrHtml = html.split(" ");
                                ArrHtml.forEach(element => {
                                    if((element+" ").length < limit_char){
                                        address += element+" ";
                                        limit_char -= (element+" ").length;
                                    }
                                });
                                
                                if(html.length > $this.state.limitChar){
                                    address += "...";
                                }
                            }
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            '<a data-toggle="tooltip" data-placement="bottom" title="Kelola" href="'+$this.state.url+'/admin/users/form/'+row.id+'" class="btn btn btn-default btn-sm"><i class="fa fa-file"></i></a>'+
                            '<button class="btn btn-default btn-sm" data-name="delete-users" data-id="'+row.id+'"><i class="fa fa-trash"></i></button>'+
                            '<button class="btn btn-default btn-sm" data-name="add-member_dept" data-toggle="modal" data-id="'+row.id+'" data-target="#modal-default"><i class="fa fa-building"></i></button>'+
                            '</div>'+
                            '</div>';
                        }
                    }
        ...
        $('#dataUsers tbody').on( 'click', 'button', async function (e) {
            if($(this)[0].dataset.name == "delete-users"){
                await Swal.fire({
            ...
            if($(this)[0].dataset.name == "add-member_dept"){
                let result = await ExternalAPI.get(
                    $this.state.url+'/api/v1/member_dept/storeByUser/'+$(this).data().id, 
                    $this.state.header);
                if(typeof(result.code) !== 'undefined'){
                    if(result.code == 200){
                        var selected = [];
                        ...
                        await $this.setState({
                            dataSelect: selected,
                            dataDept: result.data,
                            idUserDept: $(this).data().id,
                        });
                    }
                }
            }
    ...
    render() {
        ...
        return (
            <>
                ...
                <table id="dataUsers" width="100%" className="table table-bordered table-hover">
                    <thead>
                    <tr>
                        <th>Nama lengkap</th>
                        <th>Alamat</th>
...
if (document.getElementById('cpanel-users-datagrid')) {
    const app = document.getElementById('cpanel-users-datagrid');
    ReactDOM.render(<DataGrid {...app.dataset}/>, document.getElementById('cpanel-users-datagrid'));
}
