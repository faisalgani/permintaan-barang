...
export default class DataGrid extends Component {
    ...
    async componentDidMount() {
        var $this = this;
        await this.setState({
            datatable: await $('#dataSatuanBarang').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX"   : true,
                "scrollY"   : true,
                "ajax" 		: this.state.url+"/api/v1/satuan_barang/read",
                ...
                "columns": [
                    ...
                    },            
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            '<a data-toggle="tooltip" data-placement="bottom" title="Kelola" href="'+$this.state.url+'/admin/satuan_barang/form/'+row.id+'" class="btn btn btn-default btn-sm"><i class="far fa-edit"></i></a>'+
                            '<button class="btn btn-default btn-sm" data-name="delete-room" data-id="'+row.id+'"><i class="fa fa-trash-alt"></i></button>'+
                            '</div>'+
                            '</div>';
                        }
                    }
        ...
        $('#dataSatuanBarang tbody').on( 'click', 'button', async function (e) {
            if($(this)[0].dataset.name == "delete-ticket"){
                await Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        let result = await ExternalAPI.post(
                            $this.state.url+'/api/v1/satuan_barang/delete', 
                            $this.state.header, 
                            await encryptParameter({
                                id: $(this).data().id,
                            }));
    ...
    render() {
        ...
        return (
            <>
                ...
                <table id="dataSatuanBarang" width="100%" className="table table-bordered table-hover">
                    <thead>
                    <tr>
                        <th>Nama Satuan</th>
                        <th>Action</th>
...
if (document.getElementById('cpanel-satuan-barang-datagrid')) {
    const app = document.getElementById('cpanel-satuan-barang-datagrid');
    ReactDOM.render(<DataGrid {...app.dataset}/>, document.getElementById('cpanel-satuan-barang-datagrid'));
}
