...
export default class Form extends Component {
    ...
    async loadData(){
        let result = await ExternalAPI.get(
            this.state.url+'/api/v1/system_member/store/'+this.state.id, 
            this.state.header);
    ...
    async processCreate(id_user, id_group){
        var $this = this;
        return new Promise(async function(resolve) {
            let result = await ExternalAPI.post(
                $this.state.url+'/api/v1/system_member/create', 
                $this.state.header, 
    ...
    async btnSave(){
        ...
        let result = await ExternalAPI.post(
            this.state.url+'/api/v1/system_member/deleteByGroup', 
            this.state.header, 
            await encryptParameter({
                        ...
                        <div className="col-12">
                            ...
                            <select className="duallistbox" multiple="multiple" defaultValue={dataSelect}>
                                {dataMultiple.map((item) => {
                                    return <option key={item.id} value={item.id} selected={item.selected == 1?"selected":""}>{item.first_name+" "+item.last_name}</option>
                                })}
                            </select>
                            </div>
                        </div>
...
if (document.getElementById('cpanel-system_member-form')) {
    const app = document.getElementById('cpanel-system_member-form');
    ReactDOM.render(<Form {...app.dataset}/>, document.getElementById('cpanel-system_member-form'));
}
