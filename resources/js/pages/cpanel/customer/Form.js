...
export default class Form extends Component {
        ...
        await Swal.fire({
            ...
            if (result.isConfirmed) {
                let result = await ExternalAPI.post(
                    $this.state.url+'/api/v1/customer/delete', 
                    $this.state.header, 
                ...
                if(typeof(result.code) !== 'undefined'){
                    if(result.code == 200){
                        ...
                        setTimeout(() => {
                            window.location.href = $this.state.url+"/admin/customer";
                        }, 1000);
                    }else{
    ...
    async getParams(){
        ...
        obj['nama'] = $("#nama").val();
        obj['nik'] = $("#nik").val();
        obj['id_departemen'] = $("#id_departemen").val();
        return obj;
    }
    ...
    async btnSave(){
       ...
       // console.log($("#id").val());
        if(this.state.process == "create"){
            let result = await ExternalAPI.post(
                this.state.url+'/api/v1/customer/create', 
                this.state.header, 
            ...
            let result = await ExternalAPI.post(
                this.state.url+'/api/v1/customer/update', 
                this.state.header, 
                await encryptParameter(await this.getParams()));
    ...
    async componentDidMount() {
        ...
        if(this.state.parameter.nama !== ""){
            this.setState({
                nama: this.state.parameter.nama, 
            });
            $("#nama").val(this.state.parameter.nama);
        }

        if(this.state.parameter.nik !== ""){
            this.setState({
                nik: this.state.parameter.nik, 
            });
            $("#nik").val(this.state.parameter.nik);
        }

        if(this.state.parameter.id_departemen !== ""){
            this.setState({
                id_departemen: this.state.parameter.id_departemen, 
            });
            $("#id_departemen").val(this.state.parameter.id_departemen);
        }
    ...
    render() {
        ...
        const {parentList, agenda, dataSocial, phone,  social_media,dataDepartemen} = this.state;
                                ...
                                <div className="col-md-8">
                                    <div className="card card-default">
                                        <div className="card-body">
                                            <div className="form-group">
                                                <label>Name *</label>
                                                <input type="text" defaultValue={this.state.nama}  className={"form-control"} id="nama" name="nama" />
                                            </div>
                                            ...
                                            <div className="form-group">
                                                <label>NIK *</label>
                                                <input  type="text"  defaultValue={this.state.nik}  className={"form-control"} id="nik" name="nik"/>                                               
                                            </div>                            
                                        </div>
                                                ...
                                                <div className="card">
                                                    ...
                                                    <div id="collapsFour" className="collapse" aria-labelledby="stateBar" data-parent="#accordion">
                                                        <div className="card-body">
                                                            ...
                                                            <select className="form-control" id="id_departemen" name="id_departemen">
                                                                {dataDepartemen.map((item, index) => {
                                                                     return <option key={item.id} value={item.id}>{item.nama_departemen}</option>
                                                                })}
                                                            </select>
                                                        </div>
                                                    </div>
...
if (document.getElementById('cpanel-customer-form')) {
    const app = document.getElementById('cpanel-customer-form');
    ReactDOM.render(<Form {...app.dataset}/>, document.getElementById('cpanel-customer-form'));
}
