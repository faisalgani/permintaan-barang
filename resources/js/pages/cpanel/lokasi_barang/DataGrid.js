...
export default class DataGrid extends Component {
    ...
    async componentDidMount() {
        var $this = this;
        await this.setState({
            datatable: await $('#dataLokasiBarang').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX"   : true,
                "scrollY"   : true,
                "ajax" 		: this.state.url+"/api/v1/lokasi_barang/read",
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, row) {
                            ...
                            '<a data-toggle="tooltip" data-placement="bottom" title="Kelola" href="'+$this.state.url+'/admin/lokasi_barang/form/'+row.id+'" class="btn btn btn-default btn-sm"><i class="far fa-edit"></i></a>'+
                            '<button class="btn btn-default btn-sm" data-name="delete-lokasi" data-id="'+row.id+'"><i class="fa fa-trash-alt"></i></button>'+
                            '</div>'+
                            '</div>';
                        }
                    }
        ...
        $('#dataLokasiBarang tbody').on( 'click', 'button', async function (e) {
            if($(this)[0].dataset.name == "delete-lokasi"){
                await Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        let result = await ExternalAPI.post(
                            $this.state.url+'/api/v1/lokasi_barang/delete', 
                            $this.state.header, 
                            await encryptParameter({
                                id: $(this).data().id,
                            }));
    ...
    render() {
        ...
        return (
            <>
                ...
                <table id="dataLokasiBarang" width="100%" className="table table-bordered table-hover">
                    <thead>
                    <tr>
                        <th>Lokasi</th>
                        <th>Action</th>
...
if (document.getElementById('cpanel-lokasi-barang-datagrid')) {
    const app = document.getElementById('cpanel-lokasi-barang-datagrid');
    ReactDOM.render(<DataGrid {...app.dataset}/>, document.getElementById('cpanel-lokasi-barang-datagrid'));
}
