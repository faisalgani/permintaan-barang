...
export default class DataGrid extends Component {
    ...
    async componentDidMount() {
        var $this = this;
        await this.setState({
            datatable: await $('#dataBarang').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX"   : true,
                "scrollY"   : true,
                "ajax" 		: this.state.url+"/api/v1/barang/read",
                ...
                "columns": [
                    ...
                    { 
                        ...
                        "data": "barang_to_satuan.satuan",
                        "width": "20%",
                    },   { 
                        "data": "barang_to_lokasi.lokasi",
                        "width": "20%",
                    },            
                    {
                        "data": "id",
                        ...
                        "render": function (data, type, row) {
                            ...
                            '<a data-toggle="tooltip" data-placement="bottom" title="Kelola" href="'+$this.state.url+'/admin/barang/form/'+row.id+'" class="btn btn btn-default btn-sm"><i class="far fa-edit"></i></a>'+
                            '<button class="btn btn-default btn-sm" data-name="delete-barang" data-id="'+row.id+'"><i class="fa fa-trash-alt"></i></button>'+
                            '</div>'+
                            '</div>';
                        }
                    }
        ...
        $('#dataBarang tbody').on( 'click', 'button', async function (e) {
            if($(this)[0].dataset.name == "delete-barang"){
                await Swal.fire({
    ...
    render() {
        ...
        return (
            <>
                ...
                <table id="dataBarang" width="100%" className="table table-bordered table-hover">
                    <thead>
                    <tr>
                        <th>Nama Barang</th>
                        <th>Stock</th>
...
if (document.getElementById('cpanel-barang-datagrid')) {
    const app = document.getElementById('cpanel-barang-datagrid');
    ReactDOM.render(<DataGrid {...app.dataset}/>, document.getElementById('cpanel-barang-datagrid'));
}
