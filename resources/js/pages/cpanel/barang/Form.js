...
export default class Form extends Component {
        ...
        await Swal.fire({
            ...
            if (result.isConfirmed) {
                let result = await ExternalAPI.post(
                    $this.state.url+'/api/v1/barang/delete', 
                    $this.state.header, 
                ...
                if(typeof(result.code) !== 'undefined'){
                    if(result.code == 200){
                        ...
                        setTimeout(() => {
                            window.location.href = $this.state.url+"/admin/barang";
                        }, 1000);
                    }else{
    ...
    async getParams(){
        var obj = {}
        obj['id'] = $("#id").val();
        obj['nama_barang'] = $("#nama_barang").val();
        obj['stok'] = $("#stok").val();
        obj['id_satuan'] = $("#id_satuan").val();
        obj['id_lokasi'] = $("#id_lokasi").val();
        return obj;
    }
    ...
    async btnSave(){
       ...
       // console.log($("#id").val());
        if(this.state.process == "create"){
            let result = await ExternalAPI.post(
                this.state.url+'/api/v1/barang/create', 
                this.state.header, 
            ...
            let result = await ExternalAPI.post(
                this.state.url+'/api/v1/barang/update', 
                this.state.header, 
                await encryptParameter(await this.getParams()));
    ...
    async componentDidMount() {
        ...
        let result = await ExternalAPI.get(
            this.state.url+'/api/v1/satuan_barang/store/', 
            this.state.header);
        
        if(typeof(result.code) !== 'undefined'){
            if(result.code == 200){
                this.setState({
                    dataSatuan: result.data,
                });
            }
        }

        let res = await ExternalAPI.get(
            this.state.url+'/api/v1/lokasi_barang/store/', 
            this.state.header);
        ...
        if(this.state.parameter.nama_barang !== ""){
            this.setState({
                nama_barang: this.state.parameter.nama_barang, 
            });
            $("#nama_barang").val(this.state.parameter.nama_barang);
        }

        if(this.state.parameter.stok !== ""){
            this.setState({
                stok: this.state.parameter.stok, 
            });
            $("#stok").val(this.state.parameter.stok);
        }

        if(this.state.parameter.id_satuan !== ""){
            this.setState({
                id_satuan: this.state.parameter.id_satuan, 
            });
            $("#id_satuan").val(this.state.parameter.id_satuan);
        }

        if(this.state.parameter.id_lokasi !== ""){
            this.setState({
                id_lokasi: this.state.parameter.id_lokasi, 
            });
            $("#id_lokasi").val(this.state.parameter.id_lokasi);
        }
    ...
    render() {
        ...
        const {parentList, agenda, dataSocial, phone,  social_media,dataSatuan,dataLokasi} = this.state;
                                    ...
                                    <div className="card card-default">
                                        <div className="card-body">
                                            <div className="form-group">
                                                ...
                                                <input type="text" defaultValue={this.state.nama_barang}  className={"form-control"} id="nama_barang" name="nama_barang" />
                                            </div>

                                                              
                                            <div className="form-group">
                                                <label>Stok *</label>
                                                <input  type="text"  defaultValue={this.state.stok}  className={"form-control"} id="stok" name="stok"/>                                               
                                            </div>                            
                                        </div>
                                    ...
                                    <div className="card card-default">
                                        <div className="card-body">
                                            <div id="accordion">
                                                ...
                                                <div className="card">
                                                    <div className="card-header" id="stateBar">
                                                        <h5 className="mb-0">
                                                            <button className="btn btn-link" data-toggle="collapse" data-target="#collapsFour" aria-expanded="true" aria-controls="collapsFour">
                                                                Satuan
                                                            </button>
                                                        </h5>
                                                    </div>

                                                    <div id="collapsFour" className="collapse" aria-labelledby="stateBar" data-parent="#accordion">
                                                        <div className="card-body">
                                                            <label> Pilih Satuan</label>
                                                            <select className="form-control" id="id_satuan" name="id_satuan">
                                                                {dataSatuan.map((item, index) => {
                                                                     return <option key={item.id} value={item.id}>{item.satuan}</option>
                                                                })}
                                                            </select>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="card">
                                                    <div className="card-header" id="stateBar">
                                                        <h5 className="mb-0">
                                                            <button className="btn btn-link" data-toggle="collapse" data-target="#collapsOne" aria-expanded="true" aria-controls="collapsFour">
                                                                Lokasi
                                                            </button>
                                                        </h5>
                                                    </div>

                                                    <div id="collapsOne" className="collapse" aria-labelledby="stateBar" data-parent="#accordion">
                                                        <div className="card-body">
                                                            <label> Pilih Lokasi</label>
                                                            <select className="form-control" id="id_lokasi" name="id_lokasi">
                                                                {dataLokasi.map((item, index) => {
                                                                     return <option key={item.id} value={item.id}>{item.lokasi}</option>
                                                                })}
                                                            </select>
                                                        </div>
                                                    </div>
...
if (document.getElementById('cpanel-barang-form')) {
    const app = document.getElementById('cpanel-barang-form');
    ReactDOM.render(<Form {...app.dataset}/>, document.getElementById('cpanel-barang-form'));
}
